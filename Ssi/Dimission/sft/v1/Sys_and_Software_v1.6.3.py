...
if os.access(network,os.F_OK) and os.access(synetwork,os.F_OK):
        ...
        if os.access(Yishion_sys_path,os.F_OK):
            ...
            try:
                if os.access(x86,os.F_OK):
                    ...
                    print(Runtime(),"发现Program Files (x86)")
                else:
            ...
            for r in Yishion_sys_name:
                ...
                try:
                    if os.path.exists(setup_local_No86) or os.path.exists(setup_local_X86):
                        print(Runtime(),"已经安装{0}，已经成功跳过".format(Yishion_sys_name[forsn])) 
                        forsn +=1                              
                    else:
                        os.popen(str(sys_intstall_path[0])+" /s")
                        print(Runtime(),"!!正在安装{0}!!!".format(Yishion_sys_name[forsn]))
                        forsn +=1                   
                except:
    ...
    try:
        ...
        try:
            for x in updatelen:
                os.popen(str(x))
                print(Runtime(),"正在更新{0}".format(Yishion_sys_name[forsn1]))
                forsn1 +=1
        except:
            print(Runtime(),'系统更新失败，请手动检查更新')
        ...
        try:
            if os.access(x86,os.F_OK):
                ...
                print(Runtime(),"复制到32位文件夹成功")
            else:
                copyfile(banfangupdate_root,banfangpath_local)
                print(Runtime(),"复制到64位文件夹成功")
        except:
            print(Runtime(),"板房更新文件复制失败，请手动复制更新")
    except:
        print(Runtime(),"更新文件路径查询错误")

    print(Runtime(),"正在启动软件安装，请稍后！")
    time.sleep(30)
    #执行常办公软件安装

    print(Runtime(),"序列号__Ofiice2007_Sn = DBXYD-TF477-46YM4-W74MH-6YDQ8 ")
    print(Runtime(),"序列号__VNC6.0_Sn = B7SLM-7MAX5-B4M74-UTDBE-K5WFA ")
    ...
    try:       
        ...
        print(Runtime(),"office 安装错误 请手动执行！")
        print(Runtime(),"CAC注册表未成功写入！，请手动执行")
    print(Runtime(),'请关闭SN文件')
    print(Runtime(),"程序执行完成")

else:
    print(Runtime(),'请检查网络或共享设置，请手动执行软件安装，或稍后再试')
    time.sleep(10)