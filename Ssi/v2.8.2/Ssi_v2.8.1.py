...
import os,time,winreg,sqlite3,socket,sys
...
class Install_all():
    ...
    def __init__(self,localtime=localtime,Rtime=Rtime):
        self.Rtime = Rtime
...
sys_install_tables = ('eventid','hostname','clock','error','message')#通用系统库数据表索引
...
def sql_insert(eventid=None,hostname= None,clock=None,error= None,message= None): 
    ...
    except Exception as sql_insert_err:
...
def areg():
    try:
        ...
        sql_insert(clock=l_data_time.current_time,error=winreg_error,eventid=l_data_time.eventidindex,hostname=localhostname)
    ...
    try:
        ...
        sql_insert(eventid=l_data_time.eventidindex,clock=l_data_time.current_time,hostname=localhostname,error=user_uac_err)
    ...
    try:
        ...
        sql_insert(eventid=l_data_time.eventidindex,clock=l_data_time.current_time,hostname=localhostname,error=IERanges_err)
...
def localruntime():
    try:
        ...
        sql_insert(hostname=localhostname,clock=l_data_time.current_time,eventid=l_data_time.eventidindex)
    except:
        pass
...
def sys_install():    
        ...
        if os.access(Yishion_sys_path,os.F_OK):
            ...
            try:
                if os.access(x86,os.F_OK):
                    ...
                    sql_insert(clock=l_data_time.current_time,hostname=localhostname,message=ifvalues,eventid=l_data_time.eventidindex)
                else:
                ...
                sql_insert(clock=l_data_time.current_time,hostname=localhostname,error=ifosaccess_err)
            ...
            for r in Yishion_sys_name:
                ...
                try:
                    if os.path.exists(setup_local_No86) or os.path.exists(setup_local_X86):#检查要安装的系统是否已经存在
                        ...
                        forsn +=1
                        print(l_data_time.current_time,sys_install_values)
                        sql_insert(clock=l_data_time.current_time,hostname=localhostname,message=sys_install_values,eventid=l_data_time.eventidindex)                             
                    else:
                        os.popen(str(sys_intstall_path[0])+" /s")
                        sys_install_values1 = ("!!正在安装{0}!!!".format(Yishion_sys_name[forsn]))
                        forsn +=1 
                        print(l_data_time.current_time,sys_install_values1)
                        sql_insert(clock=l_data_time.current_time,hostname=localhostname,message=sys_install_values1,eventid=l_data_time.eventidindex)                                 
                    ...
                    sql_insert(clock=l_data_time.current_time,hostname=localhostname,error=sys_install_err1,eventid=l_data_time.eventidindex)
        else:
            print(l_data_time.current_time,'Yishion_sys_path文件夹异常,请确认网络正常')
            if_Yishion_sys_path_err = ('Yishion_sys_path文件夹异常,请确认网络正常')
            sql_insert(clock=l_data_time.current_time,error=if_Yishion_sys_path_err,hostname=localhostname)
    except:
        print(l_data_time.current_time,'请确认网络正常')

    waittime1 = ("正在等待更新程序启动，请稍后！")
    print(l_data_time.current_time,waittime1)
    sql_insert(clock=l_data_time.current_time,eventid=l_data_time.eventidindex,hostname=localhostname)
    ...
    try:
        ...
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=keyfile_open_ok)
    except IOError as key_file_e:
        key_file_err = ('SN文件错误或不存在')
        print(l_data_time.current_time,key_file_err)
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=key_file_e)
    ...
    try:
        ...
        try:
            for x in updatelen:
                ...
                sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=updatavalues)
        except:
            sys_updata_Err_values1 = ('系统更新失败，请手动检查更新')
            print(l_data_time.current_time,sys_updata_Err_values1)
            sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=sys_updata_Err_values1)
        #复制板房更新文件到本地
        try:
            if os.access(x86,os.F_OK):
                #复制板房更新文件到本地
                ...
                sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=copy_banfang_updata_x32_ok_values1)
            else:
                ...
                sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=copy_banfang_updata_x64_ok_values1)
        except:           
            banfang_updata_Err1_values1 = ("板房更新文件复制失败，请手动复制更新")
            print(l_data_time.current_time,banfang_updata_Err1_values1)
            sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=banfang_updata_Err1_values1)
    except:
        update_err_values1 = ("更新文件路径查询错误")
        print(l_data_time.current_time,update_err_values1)
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=update_err_values1)

    wait_startup_software = ("正在启动软件安装，请稍后!")
    print(l_data_time.current_time,wait_startup_software)
    sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=wait_startup_software)
    ...
    if os.access(software_patn,os.F_OK):
        try:            
            ...
            for z in software:
                ...
                sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=Software_install_values)
        except:
            Software_install_err = ("软件安装错误，请手动安装")
            print(l_data_time.current_time,Software_install_err)
            sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=Software_install_err)
    else:
        software_patn_err = ('请确认网络文件夹factory_Software或共享正常')
        print(l_data_time.current_time,software_patn_err)
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=software_patn_err)
...
def Other_sft_install():
    try:    
        ...
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=office2007_startup_ok)
    except:
        office2007_install_Err = ("office 安装错误 请手动执行！")
        print(office2007_install_Err)
        sql_insert(l_data_time.current_time,eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=office2007_install_Err)
    ...
    try:
        ...
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=Cac_Reg_Open_ok)
    except:
        Cac_Reg_Open_Err = ("CAC注册表未成功写入！，请手动执行")
        print(l_data_time.current_time,Cac_Reg_Open_Err)
        sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,error=Cac_Reg_Open_Err)
        
    #关闭key文件
    close_key_file = ('请关闭SN文件')
    print(l_data_time.current_time,close_key_file)
    sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=close_key_file)

    #程序执行完成
    Setup_flish = ("程序执行完成")
    print(l_data_time.current_time,Setup_flish)
    sql_insert(eventid=l_data_time.eventidindex,hostname=localhostname,clock=l_data_time.current_time,message=Setup_flish)

    #360企业版安装
    try:
        se360 = '"\\192.168.208.1\Python\factory_Software\skylarinst-winc(192.168.0.36_80).exe"'
        os.popen(se360)#单独启动se360
        print(se360,'已经启动')
    except:
        pass
#判断安装包存放路径是否正常
if __name__=='__main__':
    ...
    if os.access(network,os.F_OK) and os.access(synetwork,os.F_OK):
        ...
        sql_insert(clock=l_data_time.current_time,hostname=localhostname,error=Network_values)
        time.sleep(10)
        sys.exit()