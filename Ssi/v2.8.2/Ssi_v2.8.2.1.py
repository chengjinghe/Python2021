            ...
            for r in Yishion_sys_name:
                ...
                try:
                    if os.path.exists(setup_local_No86) or os.path.exists(setup_local_X86):#检查要安装的系统是否已经存在
                        ...
                        os.system(str(sys_intstall_path[0])+" /s")
                        forsn +=1 
...
def Other_sft_install():
    ...
    #360企业版安装
    try:
        path360 = r'\\192.168.208.1\Python\factory_Software'
        tianqing = "skylarinst-winc(192.168.0.36_80).exe"
        se360 = os.path.join(path360,tianqing)
