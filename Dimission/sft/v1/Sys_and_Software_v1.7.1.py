...
localhostname = h1.read()

def wsql(sql):
    conn = sqlite3.connect(r'\\192.168.208.1\Python\factory_Sys\instll.db')
    t1 = conn.execute(sql)
    t = t1.fetchall()
    conn.commit()
    conn.close()
...
wsql(sql='''INSERT INTO T_TIL (Project) VALUES ('本次程序启动时间{0}')'''.format(Starttime))
...
if os.access(network,os.F_OK) and os.access(synetwork,os.F_OK):
        ...
        if os.access(Yishion_sys_path,os.F_OK):
            ...
            try:
                if os.access(x86,os.F_OK):
                    ...
                    print(ifvalues)
                    wsql(sql='''INSERT INTO T_TIL (Project,TestProject,hostnmae) VALUES ('{0}','{1}','{2}')'''.format(ifvalues[0],ifvalues[1],localhostname))
                else:
            ...
            for r in Yishion_sys_name:
                ...
                try:
                    if os.path.exists(setup_local_No86) or os.path.exists(setup_local_X86):
                        ...
                        wsql(sql='''INSERT INTO T_TIL (Project,TestProject,hostname) VALUES ('{0}','{1}','{2}')'''.format(sys_install_values[0],sys_install_values[1],localhostname))
                        ...
                        wsql(sql='''INSERT INTO T_TIL (Project,TestProject,hostname) VALUES ('{0}','{1}','{2}')'''.format(sys_install_values1[0],sys_install_values1[1],localhostname))                                       
                except:
    ...
    print(waittime1)
    wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(waittime1[0],waittime1[1]))
    ...
    try:
        ...
        wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(keyfile_open_ok[0],keyfile_open_ok[1]))
    except:
    ...
    try:
        ...
        try:
            for x in updatelen:
                ...
                forsn1 +=1
                print(updatavalues)                
                wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(updatavalues[0],updatavalues[1]))  
        except:
            sys_updata_Err_values1 = (Runtime(),'系统更新失败，请手动检查更新')
            print(sys_updata_Err_values1)
            wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(sys_updata_Err_values1[0],sys_updata_Err_values1[1]))  
        ...
        try:
            if os.access(x86,os.F_OK):
                ...
                wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(copy_banfang_updata_x32_ok_values1[0],copy_banfang_updata_x32_ok_values1[1]))
            else:
                copyfile(banfangupdate_root,banfangpath_local)
                copy_banfang_updata_x64_ok_values1 = (Runtime(),"复制到64位文件夹成功")
                wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(copy_banfang_updata_x64_ok_values1[0],copy_banfang_updata_x64_ok_values1[1]))
        except:           
            banfang_updata_Err1_values1 = (Runtime(),"板房更新文件复制失败，请手动复制更新")
            print(banfang_updata_Err1_values1)
            wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(banfang_updata_Err1_values1[0],banfang_updata_Err1_values1[1]))  
    except:
        print(Runtime(),"更新文件路径查询错误")
   
    wait_startup_software = (Runtime(),"正在启动软件安装，请稍后!")
    print(wait_startup_software)
    wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(wait_startup_software[0],wait_startup_software[1]))
    ...
    if os.access(software_patn,os.F_OK):
        try:
            ...
            for z in software:
                ...
                wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(Software_install_values[0],Software_install_values[1]))
        except:
    ...
    try:    
        ...
        wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(office2007_startup_ok[0],office2007_startup_ok[1]))
        ...
        wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(Cac_Reg_Open_ok[0],Cac_Reg_Open_ok[1]))
    except:
        office2007_install_Err = (Runtime(),"office 安装错误 请手动执行！")
        print(office2007_install_Err)
        wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(office2007_install_Err[0],office2007_install_Err[1]))

        Cac_Reg_Open_Err = (Runtime(),"CAC注册表未成功写入！，请手动执行")
        print(Cac_Reg_Open_Err)
        wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(Cac_Reg_Open_Err[0],Cac_Reg_Open_Err[1]))
        
    #关闭key文件
    close_key_file = (Runtime(),'请关闭SN文件')
    print(close_key_file)
    wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(close_key_file[0],close_key_file[1]))

    #程度执行完成
    Setup_flish = (Runtime(),"程序执行完成")
    print(Setup_flish)
    wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(Setup_flish[0],Setup_flish[1]))
else:  
    Network_values = (Runtime(),'请检查网络或共享设置，请手动执行软件安装，或稍后再试') 
    print(Network_values)
    wsql(sql='''INSERT INTO T_TIL (Project,TestProject) VALUES ('{0}','{1}')'''.format(Network_values[0],Network_values[1]))
    time.sleep(10)